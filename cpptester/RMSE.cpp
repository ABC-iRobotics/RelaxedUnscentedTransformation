...
double RMSE_sep(int N, int K, const UTSettings& settings) {
  double out = 0;
  for (int n = 0; n < N; n++) {
	// Variable for simulation
	double x_true = -1;
	  ...
	  {
		{
		  Eigen::VectorXd x_(1);
		  x_(0) = x_estimated;
		  Eigen::MatrixXd Sx_(1,1);
		  Sx_(0, 0) = Sx_estimated;

		  Eigen::VectorXd x_out;
		  Eigen::MatrixXd Sx_new, temp;;
		  UT(x_, Sx_, [k](const Eigen::VectorXd& in)->Eigen::VectorXd {
			Eigen::VectorXd out(1);
			out(0) = f_scalar(in(0), 0, k);
			return out;
			}, settings, x_out, Sx_new, temp);
		  x_estimated = x_out(0);
		  Sx_estimated = Sx_(0, 0) + Sw*Sw;
		}
		//printf("Prediction Sx: %f\n", Sx_estimated);
		double Sxz, Sz, z_estimated;
		{
		  ...
		  Eigen::VectorXd z_out;
		  Eigen::MatrixXd Sz_out;
		  Eigen::MatrixXd Szx_out;

		  UT(x_, Sx_, [](const Eigen::VectorXd& in)->Eigen::VectorXd {
			Eigen::VectorXd out(1);
			out(0) = g_scalar(in(0), 0);
			return out;
			}, settings, z_out, Sz_out, Szx_out);
		  z_estimated = z_out(0);
		  Sz = Sz_out(0, 0) + Sn*Sn;
		  Sxz = Szx_out(0, 0);
		}
...
double RMSE_sep2(int N, int K, const UTSettings& settings) {
  double out = 0;
  for (int n = 0; n < N; n++) {
	  ...
	  {
		{
		  ...
		  x_estimated = x_out(0);
		  Sx_estimated = Sxw_out(0, 0) + Sw*Sw;
		}
		//printf("Prediction Sx: %f\n", Sx_estimated);
		double Sxz, Sz, z_estimated;
		{
